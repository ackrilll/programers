...
컴퓨터 바탕화면은 각 칸이 정사각형인 격자판입니다. 이때 컴퓨터 바탕화면의 상태를 나타낸 문자열 배열 wallpaper가 주어집니다. 파
일들은 바탕화면의 격자칸에 위치하고 바탕화면의 격자점들은 바탕화면의 가장 왼쪽 위를 (0, 0)으로 시작해 (세로 좌표, 가로 좌표)로 표현합니다.
...
머쓱이의 컴퓨터 바탕화면의 상태를 나타내는 문자열 배열 wallpaper가 매개변수로 주어질 때 바탕화면의 파일들을 한 번에 삭제하기 위해 최소한의 이동거리를 갖는 드래그의 시작점과
...
발상
1. 가장 왼쪽 위 파일 부터 가장 오른쪽 아래 파일까지  -> 가장 왼쪽에 있는 인덱스, 가장 위쪽에 있는 인덱스 합쳐서 result[0] , result[1]
2. 가장 왼쪽 ->result[1] , 가장 위쪽 ->result[0] 가장 오른쪽 idx+1 -> result[3] 가장 아래 idx+1 ->result[2]
*/
class Solution67{
...
public class _67바탕화면정리_ {
    public static void main(String[] args) {
        String[] wallpaper = {".#...", "..#..", "...#."};
        int[] answer = Solution67.solution(wallpaper);
